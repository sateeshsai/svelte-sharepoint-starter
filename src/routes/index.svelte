...
<svelte:boundary>
  ...
  <main class={cn("grid content-start justify-items-center")}>
    ...
    >
      <!-- <img alt="hello" src="./assets/images/svelte-machine.avif" class="absolute left-0 bottom-0 w-screen max-w-none" /> -->
      <div class={cn("hero", PAGE_UTIL_CLASSES.padding, PAGE_UTIL_CLASSES.maxWidth)}>
        ...
        {/if}
        <h1 class="mb-10 text-5xl xl:text-6xl font-thin tracking-tight" in:slide|global>
    ...
    <div bind:this={targetNode} class="wrapper bg-muted/80 dark:bg-muted/30 w-full">
      <section class={cn(PAGE_UTIL_CLASSES.padding, PAGE_UTIL_CLASSES.maxWidth, "relative overflow-hidden grid lg:grid-cols-2 gap-6 lg:gap-8")}>
        <div class="videoWrapper overflow-hidden grid rounded-lg" in:scale|global={{ start: 0 }}>
          ...
          <div class="overlay col-start-1 row-start-1 w-full h-full bg-linear-to-r from-white/90 dark:from-black/80 from-40% dark:from-30% to-transparent p-8 grid content-center">
            <h2 class="text-3xl lg:text-5xl mb-6 text-dblue-500 dark:text-dblue-bright max-w-2/3" in:slide|global={{ axis: "x", delay: 300 }}>Batteries-included!</h2>
