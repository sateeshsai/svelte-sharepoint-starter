<script lang="ts">
  ...
  let storyFileGalleryEl = $state<HTMLElement>()!;
  const inViewport = new IsInViewport(() => storyFileGalleryEl);
</script>

<section bind:this={storyFileGalleryEl} id="gallery" class="gallery grid md:grid-cols-2 xl:grid-cols-3 gap-5">
  {#if inViewport.current}
    <svelte:boundary>
      {#snippet failed(error: any, reset)}
        <ErrorBoundaryMessage customError="Error rendering story files gallery." {error} {reset} />
      {/snippet}
